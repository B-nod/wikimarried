...
function Pagination($content, $type)
{
  ...
  if (($curpage-1) > 0){
   $pageList .= "<a href='".$pagename."".($curpage-1)."/".$title.".html' title='Previous Page' class='paging'>&laquo; Prev</a>&nbsp;";
  }
	...
	for($i=1; $i<=$pages; $i++)
	{
		if($curpage == $i)
			...
			$pageList .= ' <a href="'. $pagename .''. $i .'/'.$title.'.html" class="paging">'. $i . '</a> ';
	}
	
  if (($curpage+1) <= $pages){
   $pageList .= "<a href='".$pagename."".($curpage+1)."/".$title.".html' title='Next Page' class='paging'>&nbsp;Next &raquo;</a>";
  }
  $pageList .= "</div>\n";
