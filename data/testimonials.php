...
class Testimonials 
{
	function insertTests($groupId,$title,$name,$address,$comments, $filename)
	{
		global $conn;
		$sql="INSERT INTO testimonials SET groupId='".cleanQuery($groupId)."', title='".cleanQuery($title)."', Name = '". cleanQuery($name) ."', address = '". cleanQuery($address) ."', Comments = '". cleanQuery($comments) ."', filename = '". cleanQuery($filename) ."', nDate = NOW()";
	...
	function getByGroupIdWithLimit($groupId,$limit)
	{
		...
		$sql = "SELECT * FROM testimonials WHERE groupId='$groupId' AND status=1 ORDER BY test_id DESC LIMIT $limit";
		$result = $conn->exec($sql);
		return $result;
	}
