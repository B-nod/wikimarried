<table width="100%" border="0" cellspacing="0" cellpadding="2">
  ...
  <tr bgcolor="#F1F1F1">
    ...
    <td class="tahomabold11">Enquiry button?</td>
  ...
  <?php
	...
	if ($parentId == 0)
	{
		$result = $groups->getByTypeParentId($selectedGroupType, 0);
	}
	else
	{
  ...
  <tr <?php if ($counter % 2 == 0) { echo "bgcolor='#F7F7F7'";} ?>>
    ...
    <td valign="top">
    <?php
	if($row['linkType'] == "Contents Page" || $row['linkType'] == "Trips Page"){
		$enquiry_status = $row['enquiry_button_status'];
	...
	?>
   		<?php echo $msg; ?> / <a href="manage_cms.php?id=<?php echo $row['id']; ?><?php if(isset($_GET['parentId'])){ ?>&parentId=<?php echo $_GET['parentId']; } ?>&groupType=<?php echo $row['type'];?>&edit<?php if($row['enquiry_button_status'] == 0){ ?>=s <?php } ?>"><?php echo $msg_do; ?> </a>
    <?php
	}else{
	?>
    N/A
